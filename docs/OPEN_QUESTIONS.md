...
**Resolved (LLM extraction slice):** `MatchData` generation (comparing a `ResumeExtraction` to a `JDExtraction`) will live in a dedicated future `app/services/matching.py`, not inside `extraction.py` or `eval.py`. See `ARCHITECTURE.md` §3 for the one-line rationale: it is a comparison between two extracted documents, not a single-document extraction, and `eval.py` consumes `MatchData` as a verification reference rather than producing it.
