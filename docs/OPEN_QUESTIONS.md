...
**Resolved (match/gap-analysis slice):** No. `matching.py` is an internal service only — `generate_match_data(resume_extraction, jd_extraction, *, llm_client=None) -> MatchData` — with no router and no HTTP surface. Match/gap analysis has no backing table of its own (`match_data` only persists on `GENERATED_EMAILS`), so a standalone preview endpoint would mean recomputing (and re-paying for) the same analysis a second time when the user goes on to actually generate an email from that resume/JD pair. The product goal of surfacing match/gap analysis to the user is still met — just later, via the `match_data` field on the eventual `GeneratedEmailOut` response, not as an earlier separate step. Callers (eventually `email_generation.py`) are also responsible for checking that extractions exist before invoking this service; `matching.py` itself takes already-extracted Pydantic objects and never touches the database.
