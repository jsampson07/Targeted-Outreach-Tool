...
**Resolved (email generation + eval services slice; API boundary tightened in GENERATED_EMAILS GET-by-id slice):** `EvalGates` gained `violation_detail: str | None = None` after the original schema lock — a deliberate revision, not a silent correction of an oversight. When either Tier 1 gate is `False`, the judge populates free-form text naming the specific problem; when both pass, it stays `null`. A fixed enum of violation types was considered and rejected: refine feedback needs the *specific* unsupported claim or naming mistake, and a closed category list would either stay too coarse to be useful or require constant expansion. The field is not persisted independently (rides inside `eval_breakdown` JSONB) and exists to feed `refine(email, feedback)`. Client exposure is now enforced by `EvalGatesOut` / `EvalBreakdownOut` on `GeneratedEmailOut` — the earlier "never shown to the user" claim was aspirational until that Out split; see `DATA_MODEL.md` §2.7.
