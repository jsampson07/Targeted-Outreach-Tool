...
**Phase 3 — Personalization, eval, and a working frontend (biggest remaining risk):** The LLM layer lands in slices rather than one monolith. Structured resume/JD extraction is a user-triggered step via dedicated retryable endpoints (`POST /resumes/{id}/extract`, `POST /job-descriptions/{id}/extract`) — not inline on upload — and ships ahead of match/gap analysis (`matching.py`), grounded email generation, and the rubric-based quality check. Remaining LLM work plus a thin but real React/TypeScript frontend (company resolution → discovery → resume/JD upload → extract → generated email) is still the project's highest remaining risk and its actual differentiator per the case made above. Expect to lean on AI assistance more heavily on the frontend than on the backend/LLM logic — full line-by-line understanding of the frontend is not the goal for v1; a working, demoable, honestly-labeled-WIP app is.
