...
**Phase 2 — Close the loop (current focus):** Company name resolution (`ARCHITECTURE.md` §7) first, to unblock the real product UX (user types a name, picks from candidates, rather than typing a raw domain). Then Hunter.io as the first — and, for this phase, *only* — real provider integration: mapping Hunter's actual response shape into the existing `ProviderCandidate` normalization (schema normalization doesn't disappear just because there's one provider instead of three — it's still real work, just against one real shape instead of three), and validating the already-built orchestration logic against real rate-limit behavior. Apollo and Anymail Finder are explicitly deferred past this phase (see Deferred Features table above): a second and third real provider prove integration breadth, not correctness, and are lower priority right now than getting the full pipeline working end to end.

**Phase 3 — Personalization, eval, and a working frontend (biggest remaining risk):** Structured resume/JD extraction, match/gap analysis, grounded email generation, and the rubric-based quality check — the LLM layer, still fully unbuilt, and the project's actual differentiator per the case made above. In parallel, a thin but real React/TypeScript frontend wiring the whole flow together (company resolution → discovery → resume/JD upload → generated email). Expect to lean on AI assistance more heavily here than on the backend/LLM logic — full line-by-line understanding of the frontend is not the goal for v1; a working, demoable, honestly-labeled-WIP app is.
