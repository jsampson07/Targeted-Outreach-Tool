...
- **9 SQLAlchemy models** (`app/models/`: `user.py`, `resume.py`, `job_description.py`, `company.py`, `raw_provider_result.py`, `contact.py`, `generated_email.py`, `outcome.py`, `refresh_token.py`) — schema-level correctness confirmed via the migration matching `DATA_MODEL.md` §2 field-for-field. **Confirmed this session:** models have **FK columns only** — no ORM `relationship()` associations between `User` and `RefreshToken` (or elsewhere). Auth service uses explicit queries. Adding relationships remains a separate decision.
