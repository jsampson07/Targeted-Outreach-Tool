...
**Decision (strip model-authored closing before signature append):** Immediately after `evaluate_with_retry` returns and **before** the programmatic signature append, `generated_emails.py` runs `_strip_trailing_closing(body, candidate_name)` — a deterministic, conservative strip of any model-authored trailing valediction (and an immediately following `candidate_name` line, when present). `evaluate_with_retry` continues to see the model's raw, unmodified output; the strip only touches the final draft that is about to be signed and persisted. Root cause (dogfooding): the generation/refine negative instruction ("do not include a sign-off") is not reliably followed by `claude-haiku-4-5`, so a model-written `"Best,"` stacked with the programmatic `"Best regards,\n{name}"` and shipped a visibly broken double closing in a body intended for copy-paste. A fourth eval hard gate for "no model-authored closing" was considered and **explicitly rejected**: a gate only detects/retries probabilistically via the same model that already ignored the instruction once, and does not guarantee the artifact the user copy-pastes is clean. The code-level strip is a guarantee, not a retry. Detection is intentionally narrow (last 1–3 non-blank lines; exact standalone phrase match after trimming a single trailing comma/period; no mid-sentence substring matches) because a false positive that deletes real CTA content is worse than a rare false negative that reproduces today's bug.
