...
def resume_extraction_prompt(raw_text: str) -> str:
    ...
    return (
        ...
        f"JSON Schema:\n{schema_json}\n\n"
        "Rules:\n"
        "- candidate_name: the person's own first and last name, typically "
        "from the resume header/contact block — not a reference's name, "
        "not a company name. Use null if genuinely not identifiable.\n"
        "- skills: short skill/technology strings found in the resume\n"
        "- experience: one entry per role; use null for end_date if current\n"
        "- projects: personal, academic, or hackathon projects distinct "
        "from paid/formal roles already captured under experience. A "
        "project completed during a formal role may already be described "
        "in that role's experience bullet_points — use reasonable judgment "
        "rather than forcing a rigid split; do not duplicate the same work "
        "in both sections if the resume already presents it as part of a "
        "job entry.\n"
        "- education: short strings (degree, school, year if present)\n"
        "- If a field is genuinely absent, use an empty list (or null for "
        "candidate_name / end_date), never invent employers, degrees, "
        "names, or projects\n\n"
        "Resume text:\n"
        "---\n"
        f"{raw_text}\n"
...
def matching_prompt(
    ...
    return (
        ...
        "- projects in the resume extraction are equally valid evidence for "
        "skill_matches.resume_evidence, experience_alignment.resume_evidence, "
        "and notable_resume_strengths — not just formal experience. Cite "
        "project work when it supports a JD requirement or is a notable "
        "strength.\n"
...
def email_generation_prompt(
    ...
    return (
        ...
        "- Never mention, reference, imply, or acknowledge anything listed in "
        "match_data.unmatched_jd_requirements — the email should read as "
        "entirely positive framing built from the 2-3 selected strengths, "
        "with no acknowledgment of what's missing.\n"
        ...
        "- Do NOT include any sign-off, closing, or name at the end of the "
        "body (no \"Best regards,\" \"Sincerely,\" signature block, etc.). "
        "A closing is appended programmatically after generation.\n\n"
...
def eval_prompt(
    ...
    return (
        ...
        "fails.\n"
        "- no_unprompted_gap_admission: the email must NOT proactively "
        "acknowledge, admit, apologize for, or hedge around any "
        "qualification, skill, or experience gap relative to the JD — "
        "even when that acknowledgment is factually accurate. This is a "
        "tone/strategy check (cold outreach should not volunteer "
        "weaknesses), distinct from no_unsupported_claims which only "
        "checks factual accuracy. Generalize the pattern: any unprompted "
        "gap-admission, self-disqualification, or \"I don't yet have X\" "
        "hedge fails this gate — do not require an exact phrase match.\n"
        "If any gate is false, populate violation_detail with free-form "
        "text naming the specific problem (which claim is unsupported, "
        "how the contact name/title was wrong, or which gap-admission "
        "should be removed). If all three gates pass, set "
...
def refine_prompt(email: EmailDraft, feedback: str) -> str:
    ...
    return (
        ...
        "- Do NOT include any sign-off, closing, or name at the end of the "
        "body (no \"Best regards,\" \"Sincerely,\" signature block, etc.). "
        "A closing is appended programmatically after evaluation.\n\n"
        "Feedback to address:\n"
        "---\n"
        f"{feedback}\n"
        "---\n\n"
